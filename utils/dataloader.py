...
class CustomDataLoader:
    ...
    def _read_data(self):
        ...
        n_original = len(df)
        if self.sample_ratio < 1.0 and self.data.stem.startswith('PEMS'):
            points_per_week = 7 * 24 * 12
            total_weeks = n_original // points_per_week
            sample_weeks = max(1, int(total_weeks * self.sample_ratio))

            start_week = 1
            start_idx = start_week * points_per_week
            end_idx = start_idx + sample_weeks * points_per_week
